        ...
        public override IEnumerable<ITarget> GetTargets(GameLocation location, Vector2 lookupTile)
        {
            ...
            foreach (KeyValuePair<Vector2, SObject> pair in location.objects.Pairs)
            {
                ...
                if (this.GameHelper.CouldSpriteOccludeTile(pair.Key, lookupTile))
                    yield return new ObjectTarget(this.GameHelper, pair.Value, pair.Key, () => this.BuildSubject(pair.Value, ObjectContext.World, location, knownQuality: false));
            }
            ...
            foreach (KeyValuePair<Vector2, TerrainFeature> pair in location.terrainFeatures.Pairs)
            {
                ...
                if (feature is HoeDirt { crop: not null } dirt && this.GameHelper.CouldSpriteOccludeTile(entityTile, lookupTile))
                    yield return new CropTarget(this.GameHelper, dirt, entityTile, this.Reflection, () => this.BuildSubject(dirt.crop, ObjectContext.World, dirt));
            }
        }
                ...
                case JunimoNoteMenu bundleMenu:
                    {
                        ...
                        for (int i = 0; i < bundleMenu.ingredientList.Count; i++)
                        {
                            if (bundleMenu.ingredientList[i].containsPoint(cursorX, cursorY))
                            {
                                ...
                                var item = this.GameHelper.GetObjectById(ingredient.index, ingredient.stack);
